...
/**
 * Unit tests for {@link OpenTelemetryChatModelObservationConvention}.
 */
class OpenTelemetryChatModelObservationConventionTests {

    private final OpenTelemetryGenAiOptions options = new OpenTelemetryGenAiOptions();
    private final OpenTelemetryChatModelObservationConvention convention =
            new OpenTelemetryChatModelObservationConvention(options);

    @Test
    void shouldHaveName() {
        assertThat(convention.getName())
                .isEqualTo(OpenTelemetryChatModelObservationConvention.DEFAULT_NAME);
    }
    ...
    void shouldIncludeToolDefinitionsWhenEnabled() throws JSONException {
        options.getInference().setIncludeToolDefinitions(true);
        ...
        KeyValues keyValues = convention.getHighCardinalityKeyValues(context);
        String toolDefs = findKeyValue(keyValues, GenAiMoreIncubatingAttributes.GEN_AI_TOOL_DEFINITIONS.getKey());
        JSONAssert.assertEquals("""
    ...
    void shouldNotIncludeToolDefinitionsWhenDisabled() {
        ...
        KeyValues keyValues = convention.getHighCardinalityKeyValues(context);
        assertThat(keyValues).noneSatisfy(kv ->
                assertThat(kv.getKey()).isEqualTo(GenAiMoreIncubatingAttributes.GEN_AI_TOOL_DEFINITIONS.getKey()));
    }
